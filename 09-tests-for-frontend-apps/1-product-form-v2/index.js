...
export default class ProductForm {
  ...
  getSubCategories(category) {
    return category.subcategories
      .map((subcategory) => {
        return `
        <option 
          value="${escapeHtml(category.id)}-i-${escapeHtml(subcategory.id)}"
  ...
  getImagesForSortableList() {
    return this.product.images.map((image) => {
      const div = document.createElement("div");
      div.innerHTML = this.getImageHTMLTemplate(image);
  ...
  renderProduct() {
    ...
    this.subElements.imageListContainer.append(
      new SortableList({
        items: this.getImagesForSortableList(),
      }).element
    );
    this.setSubCategory();
  ...
  getImages() {
    ...
    const imagesUl = imageListContainer.children[0].children;
